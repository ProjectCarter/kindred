 ...
 * Keep in sync with supabase/functions/_shared/weather/canonicalCondition.ts
 */
...
function canonicalFromOpenWeatherId(
  ...
  if (id === 800) {
    return { canonical: windy ? "wind" : "clear", mappingSource: "openweather_id" };
  }
  if (id === 801) {
    return {
  ...
  if (id === 804) {
    return { canonical: windy ? "wind" : "cloudy", mappingSource: "openweather_id" };
  }
  return {
