...
test("EDITION_BUILD_STAGES — ten ordered stages ending in finalize", () => {
  assert.equal(EDITION_BUILD_STAGES.length, 10);
...
test("nextEditionBuildStage — walks pipeline then returns null", () => {
  ...
  assert.equal(nextEditionBuildStage("initialize_edition"), "attach_national_daily");
...
test("isStageComplete — tracks completed stage names", () => {
  const done = ["initialize_edition", "attach_national_daily"];
