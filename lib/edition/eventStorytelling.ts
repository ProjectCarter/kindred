/**
 * Event storytelling — category voice + golden test.
...
/** How each event type should read — passed to the editorial model. */
...
/** Classify event for storytelling voice before writing. */
export function classifyEventStoryType(input: {
  name: string;
  venue: string;
  category?: LocalEventCategory | null;
}): EventStoryType {
  const hay = `${input.name} ${input.venue}`.toLowerCase();
...
export function extractEventIdentityTokens(input: {
  name: string;
  venue: string;
}): string[] {
  const venue =
    input.venue.trim() && input.venue.trim().toLowerCase() !== "venue tba"
      ? input.venue.trim()
      : "";
  const fromName = significantTokens(input.name);
  const fromVenue = significantTokens(venue);
  return [...new Set([...fromName, ...fromVenue])].slice(0, 8);
}

/** Listing-only openers that fail the storytelling standard. */
const LISTING_ONLY_OPENER =
  /^(?:[\w\s,'-]+ is (?:scheduled|listed|on|happening)|an event is happening|join us for|come out for)\b/i;

/**
 * Golden test: without the title, would this copy still fit ONLY this event?
 * Requires identifiable tokens from the verified name/venue in the copy.
 */
