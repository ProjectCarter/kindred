// Kindred — refresh-today-in-history
// Replaces stale Today in History copy in an existing ready edition.
...
import { refreshTodayInHistoryForEdition } from "../_shared/history/refreshTodayInHistory.ts";
...
Deno.serve(async (req) => {
  try {
    ...
    const result = await refreshTodayInHistoryForEdition(admin, {
    ...
    console.error("[refresh-today-in-history] failure", err);
    return Response.json(
